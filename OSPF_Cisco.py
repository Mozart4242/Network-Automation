...
def ospf(HOST,user,password,ensec):
	nw = input("In which network do you want to run OSPF? >> ")
	wildmask = input("Enter Wildcard mask: >> ")
	pid = input("Proccess id? >> ")
	areaid = input("What Area? >> ")
	...
	tn.write(b"conf t\n")
	tn.write(b"router ospf " + pid.encode('ascii') + b"\n")
	tn.write(b"network " + nw.encode('ascii') + b" " + wildmask.encode('ascii') + b" area " + areaid.encode('ascii') + b"\n")
	tn.write(b"end\n")
	tn.write(b"exit\n")
